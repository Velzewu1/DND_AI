						...
						<div className="grid md:grid-cols-4 gap-6">
							...
							<div className="space-y-3">
								...
								<input
									...
									className="w-full bg-black/80 border border-green-500/40 text-green-200 px-3 py-2 font-serif text-sm focus:border-gold-500 focus:outline-none transition-all rounded"
								/>
								<select
									value={config.tone}
									...
									<div className="flex gap-1">
										{[1, 2, 3, 4, 5].map(num => (
											<button
												...
												className={`flex-1 px-2 py-1 border font-mono text-xs transition-all ${
													config.acts === num
							...
							<div className="space-y-3">
								...
								<div className="font-mono text-xs text-green-300 space-y-1 mb-3 bg-black/40 p-3 border border-green-500/20 rounded">
									<div className="flex justify-between">
										<span>Acts:</span>
										<span className="text-gold-light">{config.acts}</span>
									</div>
									<div className="flex justify-between">
										<span>Scenes:</span>
										<span className="text-gold-light">{config.acts * config.scenesPerAct}</span>
									</div>
									<div className="flex justify-between">
										<span>Ready:</span>
										<span className={config.setting.trim() ? "text-green-400" : "text-red-400"}>
											{config.setting.trim() ? "YES" : "NO"}
										</span>
									</div>
								</div>
								<button
									...
									className={`w-full px-4 py-2 border font-mono text-xs uppercase tracking-wider transition-all rounded ${
